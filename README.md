...
- [Архитектурная философия](#архитектурная-философия)
...
Это **криптографический фильтр реальности**, а не message broker, SIEM или data lake.

## Варианты реализации
Используйте, когда нужны:
...
```mermaid
graph TD
    A["Недоверенные источники<br>(Telegram, API, сенсоры)"]
    B["Nekhebet Core<br>Zero-trust verification<br>Ed25519 + RFC 8785"]
    C["Rejected events"]
    D["Verified events"]
    E["Nekhebet Store<br>PostgreSQL + LMDB<br>Append-only ledger"]
    F["LISTEN / NOTIFY"]
    G["Omen Display<br>Real-time pipeline<br>WebSocket + enrichment"]
    H["Display clients<br>24/7 ambient display"]
    I["Charon Vessel<br>Atomic file rotation<br>POSIX atomic ops"]
    A --> B
    B --> C
    B --> D
    D --> E
